function [X, Y, i] = PowerARMC1(O, A, mu, para )
% variables: X, Y, O
% model-par: lambda, mu
% algor-par
%
...
while mu < mu_max
    ...
    % update low rank matrix X
    Xmedia = L - 1/(tau1)*A.*(L + S - O);
    R = filterBase(V1, V0, 1e-6);
    R = R(:,1:min(size(R,2), maxR));
    [Q, ~] = powerMethod(Xmedia, R, 3, 1e-5);
    hZ = Q'*Xmedia;
    ...
    %
    Xpre = X; Ypre = Y;
    %XegSvS = Xsvs+beta*(Xsvs-Xsvs_pre);   % meanings
    %Xsvs_pre = Xsvs;
    SS = svd(Xeg, 'econ');
    % how to choose L and S
    obj0(i) = getObjRMC(X, diag(Xsvs), Y, O, A, lambda, mu, regType);
    obj1(i) = getObjRMC(Xeg, SS, Yeg, O, A, lambda, mu, regType);
    ...
    output.err = err(1:i);
    output.obj0 = obj0(1:i);
    output.obj1 = obj1(1:i);
end
